...
def validate_csv(df: pd.DataFrame) -> dict:
    ...
    # Check numeric values in quantity
...
def get_sales_stats(product_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # Check if sufficient for forecast
    sufficient_for_forecast = total_rows >= 90
    
    # Detect seasonality (simple approach: check weekly variance)
    ...
    if total_rows > 90:
        try:
            ...
            if len(weekly) > 4:
                ...
                # If weekly std is significant relative to mean, likely seasonal
